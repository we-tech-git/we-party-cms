...
export function ActionBar({ isSubmitting, error }: ActionBarProps) {
  ...
  return (
    <div
      className="sticky bottom-4 z-40 flex items-center gap-[14px] flex-wrap rounded-[20px] px-[18px] py-[14px] mt-1"
      style={{
    ...
    >
      <span className="flex items-center gap-2 font-semibold text-[13.5px]" style={{ color: 'var(--ink-soft)' }}>
        {error ? (
          ...
          <>
            ...
            <span>Tudo pronto pra publicar!</span>
          </>
        ) : (
          <>
            ...
            Adicione um nome e uma data para publicar.
          </>
        )}
      </span>
      ...
      <div className="ml-auto flex gap-3">
        <button
          type="button"
          disabled
          className="flex items-center gap-2 rounded-[14px] px-6 py-[14px] font-extrabold text-[14px] border transition-all disabled:opacity-40 disabled:cursor-not-allowed"
